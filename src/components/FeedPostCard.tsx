...
import { MessageCircle, Bookmark, Share, AlertTriangle, BookmarkCheck, Eye, EyeOff, Trash2 } from 'lucide-react';
...
const FeedPostCard = ({ post }: FeedPostCardProps) => {
  ...
  const handleDeletePost = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['discovery-posts'] });
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-sm border mb-6">
      <div className="flex items-center gap-3 mb-4">
        <div 
          className={`flex items-center gap-3 flex-1 ${!post.is_anonymous ? 'cursor-pointer hover:opacity-80' : ''}`}
